...
class NoteEditorPage extends ConsumerStatefulWidget {
  final int? noteId; const NoteEditorPage({super.key, this.noteId});
...
class _S extends ConsumerState<NoteEditorPage>{
  ...
  @override Widget build(BuildContext c){
    final st=ref.watch(editorProvider); final ctrl=ref.read(editorProvider.notifier);
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(icon: const Icon(Icons.palette), onPressed: ()=>_pickColor(c, ctrl)),
          IconButton(icon: const Icon(Icons.save), onPressed: () async {
            final id=await ref.read(notesRepoProvider).upsert(
              id: widget.noteId, title: st.title, body: st.body, color: st.color);
            await ref.read(revisionsRepoProvider).add(id, ctrl.snapshotJson());
      ...
      body: Padding(
        ...
        child: Column(children:[
          TextField(
            controller:_title, decoration: const InputDecoration(hintText:'Título', border: InputBorder.none),
            onChanged:(v)=>ctrl.set(title:v)),
          Expanded(child: TextField(
            controller:_body, maxLines:null, decoration: const InputDecoration(hintText:'Escreve…', border: InputBorder.none),
            onChanged:(v)=>ctrl.set(body:v))),
          Row(children:[
            TextButton.icon(icon: const Icon(Icons.check_box_outlined), label: const Text('Checklist'),
  ...
  void _toggleChecklist(EditorCtrl ctrl){
    ...
    final lines=t.split('\n');
    if(lines.isEmpty){ ctrl.set(body: '- [ ] '); return; }
    final i=lines.length-1; final L=lines[i];
    final toggled = L.startsWith('- [ ] ')
      ? '- [x] ${L.substring(6)}'
      : L.startsWith('- [x] ')
        ? '- [ ] ${L.substring(6)}'
        : '- [ ] $L';
    lines[i]=toggled; ctrl.set(body: lines.join('\n'));
  }
  void _pickColor(BuildContext c, EditorCtrl ctrl){
    ...
    showModalBottomSheet<void>(context:c, builder:(_)=>GridView.count(
      crossAxisCount:6, padding: const EdgeInsets.all(12),
      children: colors.map((v)=>InkWell(
        onTap:(){ ctrl.set(color:v); Navigator.pop(c); },
        child: Card(color: Color(v)))).toList(),
    ));
  }
}
